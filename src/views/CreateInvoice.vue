  ...
  <v-container class="container">
    <v-form class="form" ref="form" lazy-validation>
      ...
      >
        ...
        ></v-text-field>
        <v-btn class="delete-btn" @click="removeProduct(product.id)"
      ...
      <v-btn
        :disabled="!valid"
        class="indigo create-btn btn darken-1 white--text"
        @click="saveInvoice"
...
export default {
  ...
  data() {
    return {
      ...
      finishedInvoice: {
        clientName: "",
        invoiceDate: new Date(Date.now()).toLocaleDateString(
          "en-us",
          this.dateOption
        ),
        invoiceProducts: [],
  ...
  methods: {
    ...
    saveInvoice() {
      ...
      this.finishedInvoice.invoiceProducts = this.currentProducts.map(
        (product) => {
          this.subTotal =
            this.subTotal + product.productPrice * product.productQty;
          return product;
        }
      );
      this.finishedInvoice.subTotal = this.subTotal;
      console.log(this.subTotal);
      this.finishedInvoice.total =
        this.discount === 0
          ? this.subTotal
